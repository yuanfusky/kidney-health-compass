...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: t("KidneyTrack 肾脏健康记录 — 属于患者自己的慢性肾病档案") },
      {
        name: "description",
        content: t(
      ...
      { property: "og:title", content: t("KidneyTrack 肾脏健康记录") },
      {
        property: "og:description",
        content: t("把分散在各家医院的肾脏检查资料整理成一条属于你自己的健康时间线。"),
      },
...
function getFeatures(t: ReturnType<typeof useT>) {
  return [
    { Icon: FileCheck2, title: t("报告集中管理"), desc: t("拍照或上传 PDF，自动识别化验数值，由你本人确认后入档。") },
    { Icon: LineChart, title: t("长期趋势可视化"), desc: t("肌酐、eGFR、尿蛋白、血压随时间变化，用药事件同图标注。") },
    { Icon: Stethoscope, title: t("复诊摘要"), desc: t("一键生成近 3/6/12 个月的关键变化，以及想向医生确认的问题。") },
  ];
}
      ...
      <div className="mx-auto max-w-3xl px-5 py-14 md:py-20">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Button asChild size="lg" className="h-13 px-7 text-base">
            <Link to="/auth">{checking ? t("进入 KidneyTrack") : t("登录 / 注册")}</Link>
          </Button>
          <Button asChild variant="outline" size="lg" className="h-13 px-7 text-base">
            <Link to="/auth">{t("查看演示档案")}</Link>
          </Button>
        </div>
