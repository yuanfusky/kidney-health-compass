...
export const Route = createFileRoute("/_authenticated/reports/$reportId")({
  head: () => ({
    meta: [
      { title: "报告详情 — KidneyTrack" },
      { name: "description", content: "查看原始报告、AI 识别值与你确认后的数值。" },
      { property: "og:title", content: "报告详情 — KidneyTrack" },
      { property: "og:description", content: "每一项数值都保留来源报告与确认记录。" },
...
function ReportDetail() {
  ...
  return (
    <>
      <PageHeader
        title="报告详情"
        subtitle={data ? `${data.report.report_date} · ${data.report.hospital ?? ""}` : "加载中…"}
        showProfile={false}
        action={
          <Button asChild variant="ghost" size="icon" className="size-11">
            <Link to="/reports" aria-label="返回报告列表">
        ...
        {data ? (
          <>
            <section className="kt-card p-4">
              <p className="text-[15px] text-muted-foreground">报告类型</p>
              <p className="text-[19px] font-semibold">{data.report.report_type}</p>
              <div className="mt-3 grid grid-cols-2 gap-3 text-[15px]">
                <div>
                  <p className="text-muted-foreground">检查日期</p>
              ...
              {data.signedUrl ? (
                ...
                <p className="mt-4 rounded-lg bg-surface p-3 text-[14px] text-surface-foreground">
                  这份报告没有附带原始文件（演示数据或手动录入）。
                </p>
              )}
            </section>

            <section className="kt-card p-4">
              <h2 className="text-[18px] font-semibold">检查数值</h2>
                    ...
                    <li key={l.id} className="py-3">
                      ...
                      <p className="mt-1 text-[13px] text-muted-foreground">
                        ...
                        {l.reference_min !== null || l.reference_max !== null
                          ? ` · 参考范围 ${l.reference_min ?? "—"} – ${l.reference_max ?? "—"} ${l.original_unit ?? ""}`
                          : ""}
                      </p>
                      <p className="mt-1 text-[13px] text-muted-foreground">
                        {l.extracted_value !== null
                          ? `AI 识别值 ${l.extracted_value}${l.confidence ? ` · 可信度 ${Math.round(Number(l.confidence) * 100)}%` : ""} · 已由本人确认`
                          : "由本人手动录入"}
                      </p>
                    </li>
                  );
