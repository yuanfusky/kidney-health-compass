...
export const Route = createFileRoute("/_authenticated/profile")({
  head: () => ({
    meta: [
      { title: t("个人中心 — KidneyTrack") },
      { name: "description", content: t("患者档案、家属共享、隐私设置、数据导出与账号删除。") },
      { property: "og:title", content: t("个人中心 — KidneyTrack") },
      { property: "og:description", content: t("您的健康数据属于您本人，未经授权不会与第三方共享。") },
...
function ProfilePage() {
  ...
  return (
    <>
      <PageHeader title={t("个人中心")} showProfile={false} />

      <div className="mx-auto max-w-3xl space-y-5 px-4 py-5 md:px-8">
        <section className="kt-card p-5">
          <p className="text-[22px] font-semibold">{patient?.name ? t(patient.name) : "—"}</p>
          <p className="mt-1 text-[15px] text-muted-foreground">
            {patient?.sex ? t(patient.sex) : ""} · {patient?.birth_year}{t(" 年出生")}
          </p>
          <p className="mt-2 text-[16px]">{patient?.diagnosis ? t(patient.diagnosis) : ""}</p>
          <p className="text-[14px] text-muted-foreground">
            {t("确诊日期 {date}", { date: patient?.diagnosis_date ?? "" })}
          </p>
        ...
        <nav className="kt-card divide-y divide-border">
          <Link to="/biopsy" className="flex items-center gap-3 p-4 transition-colors hover:bg-muted/40">
            ...
            <span className="flex-1 text-[17px] font-medium">{t("IgA肾病病理")}</span>
            <ChevronRight className="size-5 text-muted-foreground" />
          </Link>
          <Link to="/family" className="flex items-center gap-3 p-4 transition-colors hover:bg-muted/40">
            <Users className="size-5 text-primary" />
            <span className="flex-1 text-[17px] font-medium">{t("家属管理")}</span>
            <ChevronRight className="size-5 text-muted-foreground" />
          </Link>
        </nav>
        ...
        <section className="kt-card p-5">
          ...
          <p className="mt-2 text-[14px] leading-relaxed text-muted-foreground">
            {t("报告文件保存在私有存储空间，只有你和你授权的家属可以访问；访问链接为临时有效链接。")}
          </p>

          <div className="mt-4 space-y-3">
            <label className="flex items-center justify-between gap-3">
              <span className="text-[16px]">{t("允许家属查看趋势图")}</span>
              <Switch checked={shareTrends} onCheckedChange={setShareTrends} />
            </label>
            <label className="flex items-center justify-between gap-3">
              <span className="text-[16px]">{t("允许家属查看原始报告")}</span>
        ...
        <section className="kt-card p-5">
          <h2 className="text-[18px] font-semibold">{t("数据与账号")}</h2>
          <div className="mt-3 space-y-2.5">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-xl">{t("删除账号与数据？")}</AlertDialogTitle>
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel className="h-12 text-base">{t("取消")}</AlertDialogCancel>
                  <AlertDialogAction
                    className="h-12 text-base"
                    onClick={async () => {
                      toast.success(t("删除请求已提交，账号将在退出后处理"));
                      await signOut();
                    }}
                  >
                    {t("确认删除")}
