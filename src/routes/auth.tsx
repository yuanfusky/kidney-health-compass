...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "登录 KidneyTrack — 肾脏健康记录" },
      { name: "description", content: "登录或注册 KidneyTrack，管理属于你自己的慢性肾病健康档案。" },
      { property: "og:title", content: "登录 KidneyTrack" },
      { property: "og:description", content: "登录后查看你的肾脏健康时间线、趋势与复诊摘要。" },
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) navigate({ to: "/home", replace: true });
        else toast.success("注册成功，请查看邮箱确认邮件后登录。");
      } else {
    ...
    <main className="grid min-h-screen place-items-center bg-background px-5 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="kt-card mt-5 p-6">
          <h1 className="text-2xl font-semibold">{mode === "signin" ? "登录" : "创建账号"}</h1>
